...
export default async function DashboardPage() {
  ...
  const recentShifts = await prisma.staffShift.findMany({
    include: {
      staff: true,
    },
    orderBy: {
  ...
  type ShiftRecord = (typeof recentShifts)[number];
          ...
          {todayAttendance.length === 0 ? (
            ...
            todayAttendance.map((record: AttendanceRecord) => (
              <div key={record.id} className="rounded-xl border p-3">
                ...
                <p className="text-sm text-gray-500">
                  {record.class?.className ?? "Unknown class"}
                </p>
      ...
      <div className="rounded-xl border p-4">
        ...
        <div className="mt-4 space-y-3">
          {recentShifts.length === 0 ? (
            ...
            recentShifts.map((shift: ShiftRecord) => (
              <div key={shift.id} className="rounded-xl border p-3">
                <p className="font-medium">
                  {shift.staff
                    ? `${shift.staff.firstName} ${shift.staff.lastName}`
